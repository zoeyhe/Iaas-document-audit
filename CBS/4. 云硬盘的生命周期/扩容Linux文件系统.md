云硬盘是云上可扩展的存储设备，用户可以在创建云硬盘后随时扩展其大小，以增加存储空间，同时不失去云硬盘上原有的数据。要达到扩容并使用扩容空间的目的，用户在扩容实体云硬盘大小之后，还需要扩展其上的文件系统以识别新近可用的空间。您可以根据下面的步骤进行
...
如果云硬盘已进行分区，则必须在调整文件系统前对分区进行操作。检查您的卷分区是否需要调整大小：使用 `lsblk` 命令列出连接到实例的块储存设备。

```
lsblk

NAME    MAJ:MIN RM SIZE RO TYPE MOUNTPOINT
vda    202:0    0  30G  0 disk
└─vda1 202:1    0  30G  0 part /
vdb    202:80   0  35G  0 disk
└─vdb1 202:81   0   8G  0 part

```
这里 `/dev/vdb1` 是一个位于 35G 的设备上的 8G 的分区，并且该设备没有其他分区，所以必须调整分区。调整分区大小之后，可以继续扩展文件系统以占用分区上的所有空间。
